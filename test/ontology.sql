...
SELECT lives_ok('INSERT INTO standoff.ontology_resource
...
SELECT throws_ok('INSERT INTO standoff.ontology_resource
       			(ontology, local_name) VALUES
			(currval(''standoff.ontology_id_seq''), ''Beispiel'')');
...
SELECT is(version, 'v0.3') FROM standoff.ontology o, standoff.ontology_resource r
       WHERE r.local_name = 'Konzept' AND r.ontology = o.id;


SELECT set_eq('SELECT version FROM standoff.ontology o, standoff.ontology_resource r
       		      WHERE r.local_name = ''Beispiel'' AND r.ontology = o.id',
	      ARRAY['v0.3', 'v0.2']) ;


SELECT is(qualified_name, 'http://arb.de/ontology/arb/Konzept')
       FROM standoff.markup_resource
