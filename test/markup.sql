...
SELECT lives_ok('INSERT INTO standoff.markup
       			(document, id, term) VALUES
			(currval(''standoff.document_id_seq''), md5(''testId1'')::uuid, currval(''standoff.term_id_seq'')-2)');
...
SELECT throws_ok('INSERT INTO standoff.markup
       			 (document, id, term) VALUES
			 (currval(''standoff.document_id_seq''), md5(''testId2'')::uuid, currval(''standoff.term_id_seq''))',
		 '23514',
...
SELECT lives_ok('INSERT INTO standoff.markup_range
			...
			(currval(''standoff.document_id_seq''), md5(''testId1'')::uuid, md5(''testId1.1'')::uuid, ''[23, 42]'', ''[123, 142]''),
			(currval(''standoff.document_id_seq''), md5(''testId1'')::uuid, md5(''testId1.2'')::uuid, ''[111, 152]'', ''[211, 252]'')');
...
SELECT throws_ok('INSERT INTO standoff.markup_range
       	  		 (document, markup, id, text_range, source_range) VALUES
			 (currval(''standoff.document_id_seq''), md5(''testId1'')::uuid, md5(''testId1.fail'')::uuid, ''[23, 42]'', ''[1000, 9999]'')',
			 '23505',
			 'duplicate key value violates unique constraint "markup_range_markup_text_range_key"');

-- Neither with the same value on source_range.
SELECT throws_ok('INSERT INTO standoff.markup_range
       	  		 (document, markup, id, text_range, source_range) VALUES
			 (currval(''standoff.document_id_seq''), md5(''testId1'')::uuid, md5(''testId1.fail'')::uuid, ''[1212, 1234]'', ''[123, 142]'')',
			 '23505',
...
SELECT is(count(*)::integer, 1) FROM standoff.markup WHERE document = currval('standoff.document_id_seq');
...
SELECT lives_ok('INSERT INTO standoff.markup_range
       			(document, markup, id, text_range, source_range) VALUES
			(currval(''standoff.document_id_seq''), md5(''testId1'')::uuid, md5(''testId1.3'')::uuid, ''[186, 192]'', ''[286, 292]'')');
...
SELECT lives_ok('UPDATE standoff.markup SET privilege = 390 WHERE document = currval(''standoff.document_id_seq'')');
...
SELECT lives_ok('UPDATE standoff.markup SET privilege = 432 WHERE document = currval(''standoff.document_id_seq'')');
...
SELECT lives_ok('UPDATE standoff.markup SET privilege = 391 WHERE document = currval(''standoff.document_id_seq'')');
